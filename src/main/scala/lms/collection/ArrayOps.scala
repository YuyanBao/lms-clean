...
trait ArrayOps { b: Base =>
  ...
  def RefArray[T: Manifest](x: Rep[Int]): Rep[Array[T]] = {
    Wrap[Array[T]](Adapter.g.reflectMutable("RefArray", Unwrap(x)))
  }
  ...
  implicit class ArrayOps[A: Manifest](x: Rep[Array[A]]) {
    def apply(i: Rep[Int]): Rep[A] =
      x match {
        case Wrap(_) =>
          Wrap[A](
            Adapter.g.reflectRead("array_get", Unwrap(x), Unwrap(i))(Unwrap(x))
          )
        case EffectView(x, base) =>
          Wrap[A](
            Adapter.g.reflectRead("array_get", Unwrap(x), Unwrap(i))(
              Unwrap(base)
            )
          )
      }
    def &(i: Rep[Int]): Rep[A] =
      x match {
        case Wrap(_) =>
          Wrap[A](
            Adapter.g.reflectRead("array_get_ref", Unwrap(x), Unwrap(i))(
              Unwrap(x)
            )
          )
        case EffectView(x, base) =>
    ...
    def update(i: Rep[Int], y: Rep[A]): Unit =
      x match {
        case Wrap(_) =>
          Adapter.g.reflectWrite("array_set", Unwrap(x), Unwrap(i), Unwrap(y))(
            Unwrap(x)
          )
        case EffectView(x, base) =>
          Adapter.g.reflectWrite("array_set", Unwrap(x), Unwrap(i), Unwrap(y))(
    ...
    def length: Rep[Int] =
      Wrap[Int](Adapter.g.reflect("array_length", Unwrap(x)))
    def slice(s: Rep[Int], e: Rep[Int]): Rep[Array[A]] =
      EffectView[Array[A]](
        Wrap[Array[A]](
          Adapter.g.reflect("array_slice", Unwrap(x), Unwrap(s), Unwrap(e))
        ),
        x
  ...
  implicit class CharArrayOps(x: Rep[Array[Char]]) {
    def ArrayOfCharToString(): Rep[String] = {
      Wrap[String](
        Adapter.g.reflectRead("array-of-char-to-string", Unwrap(x))(Unwrap(x))
      )
    }
  }

  trait LongArray[+T]
  def NewLongArray[T: Manifest](
      x: Rep[Long],
      init: Option[Int] = None
  ): Rep[LongArray[T]] =
    init match {
      case Some(v) =>
        Wrap[LongArray[T]](
          Adapter.g.reflectMutable("NewArray", Unwrap(x), Backend.Const(v))
        )
      case _ =>
        Wrap[LongArray[T]](Adapter.g.reflectMutable("NewArray", Unwrap(x)))
    }
  ...
  implicit class LongArrayOps[A: Manifest](x: Rep[LongArray[A]]) {
    ...
    def length: Rep[Long] =
      Wrap[Long](Adapter.g.reflect("array_length", Unwrap(x)))
    ...
    def resize(s: Rep[Long]): Rep[LongArray[A]] =
      Wrap[LongArray[A]](
        Adapter.g.reflectRealloc("array_resize", Unwrap(x), Unwrap(s))(
          Unwrap(x)
        )
      )
    def free: Unit = Adapter.g.reflectFree("array_free", Unwrap(x))(Unwrap(x))
...
trait StackArrayOps extends ArrayOps { b: Base =>
  ...
  def StackArray[T: Manifest](xs: Rep[T]*): Rep[Array[T]] = {
    Wrap[Array[T]](
      Adapter.g.reflectMutable("StackArray", xs.map(Unwrap(_)): _*)
    )
  }
}
  ...
  override def traverse(n: Node): Unit =
    n match {
      case n @ Node(s, "NewStackArray", List(x), _) =>
        val tpe = remap(
          typeMap.get(s).map(_.typeArguments.head).getOrElse(manifest[Unknown])
        )
