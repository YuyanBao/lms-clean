...
object Backend {
  ...
  case class EffectSummary(
    ...
    def repr(f: Exp => String) = {
      val keyString =
        (if (rkeys.nonEmpty) rkeys.toSeq.sorted.map(f).mkString(" ") + " "
         else "") +
          (if (wkeys.nonEmpty) wkeys.toSeq.sorted.map(f(_) + "*").mkString(" ")
           else "")
      val depString =
        (if (sdeps.nonEmpty) sdeps.toSeq.sorted.map(f).mkString(", ")
         else "_") + " | " +
          (if (hdeps.nonEmpty) hdeps.toSeq.sorted.map(f).mkString(", ")
           else "_")
      "[" + keyString + ": " + depString + "]"
    }
    ...
    def isEmpty = isPure
    lazy val isPure =
      sdeps.isEmpty && hdeps.isEmpty && rkeys.isEmpty && wkeys.isEmpty
    lazy val hasSimpleEffect =
      wkeys.exists(key => key.isInstanceOf[Const] && key != STORE)
...
class GraphBuilder {
  ...
  def findDefinition(op: String, as: Seq[Def]): Option[Node] =
    globalDefsReverseCache.get((op, as))
  ...
  def reflectRead(s: String, as: Def*)(efKeys: Exp*) =
    reflectEffect(s, as: _*)(efKeys: _*)()
  def reflectWrite(s: String, as: Def*)(efKeys: Exp*) =
    reflectEffect(s, as: _*)()(efKeys: _*)
  ...
  //      x13: free(x3) // hdeps x12
  def reflectFree(s: String, as: Def*)(efKeys: Exp) =
    reflectEffect(s, as: _*)()(efKeys, STORE)
  def reflectRealloc(s: String, as: Def*)(efKeys: Exp) =
    reflectEffect(s, as: _*)(STORE)(efKeys, STORE)
  ...
  def reflectEffect(s: String, as: Def*)(
      readEfKeys: Exp*
  )(writeEfKeys: Exp*): Exp = {
    ...
    rewrite(s, as.toList) match {
      case Some(e) =>
        e // found optimization (resulting in pure expressions only)
      case None => // no available optimization
        ...
        val (reads, _writes) =
          ((latent_ref ++ readEfKeys).toSet, (latent_wef ++ writeEfKeys).toSet)
        // FIXME(feiw) get this special case refactored.
        val writes =
          if (s == "reset1") _writes filter (_ != stub.Adapter.CPS) else _writes
        ...
        if (reads.nonEmpty || writes.nonEmpty) {
          ...
          for (key <- reads) {
            val (lw, lrs) =
              curEffects.getOrElse(key, (curBlock, Nil)) // FIXME really?
            curEffects += key -> (lw, res :: lrs)
            if (key == STORE)
              curEffects += res -> (res, Nil) // Needed to notify res was defined locally
          } // Do we want it?
          for (key <- writes) { curEffects += key -> (res, Nil) }
          reflectOwnership(s, sm, as: _*)
  ...
  def checkOwnership(s: String, as: Def*): Unit = {
    s match {
      case "var_new" =>
  ...
  def checkWritable(n: Def): Unit = {
    n match {
      case n @ Sym(_) =>
        if (
          !globalOwnership
            .contains(n) || globalOwnership.getOrElse(n, -0.1) != 1.0
        )
          throw new Exception(s"${n} is not writtable")
    }
  }
    ...
    n match {
      case n @ Sym(_) =>
        if (
          ...
          throw new Exception(s"${n} is not readable")
      case _ =>
    }
  }

  // def checkReadable(n: Node): Unit = {
  //   n match {
  ...
  def getOwnership(n: Def): Double = {
    n match {
      case n @ Sym(_) => globalOwnership.getOrElse(n, 0.5)
      case _          => 0.5
    }
  }
  def updOwnership(n: Def, o: Double): Unit = {
    n match {
  ...
  def gatherEffectDeps(
      reads: Set[Exp],
      writes: Set[Exp],
      s: String,
      as: Def*
  ): (Set[Sym], Set[Sym]) = {
    val (prevHard, prevSoft) =
      (new mutable.ListBuffer[Sym], new mutable.ListBuffer[Sym])
  ...
  def getFunctionLatentEffect(
      f: Exp
  ): ((Set[Exp], Set[Exp]), (Set[Int], Set[Int]), Option[Exp]) =
    findDefinition(f) match {
      ...
      case Some(Node(_, "λforward", xf :: Const(arity: Int) :: Nil, _)) =>
        ...
        (
          (Set[Exp](), Set[Exp](Const("CTRL"))),
          (0.until(arity).toSet, 0.until(arity).toSet),
          None
        )
      ...
      case Some(Node(_, "@", (f: Sym) +: args, _)) =>
        ...
        val ((rk2, wk2), (prk2, pwk2), f_res_res) = getFunctionLatentEffect(
          f_res
        )
      ...
      case Some(
            Node(
              _,
              "?",
              c :: Block(ins, out, ein, eout) :: Block(
                ins2,
                out2,
                ein2,
                eout2
  ...
  def getApplyLatentEffect(
      f: Sym,
      args: Def*
  ): ((Set[Exp], Set[Exp]), Option[Exp]) = {
  ...
  def reify(f: (Exp, Exp, Exp) => Exp): Block =
    reify(3, xs => f(xs(0), xs(1), xs(2)))
  def reify(f: (Exp, Exp, Exp, Exp) => Exp): Block =
    reify(4, xs => f(xs(0), xs(1), xs(2), xs(3)))

  case class BlockEffect(
      var map: Map[Exp, (Sym, List[Sym])],
      prev: BlockEffect
  ) {
    def get(key: Exp): Option[(Sym, List[Sym])] =
      if (prev != null) map.get(key) orElse prev.get(key) else map.get(key)
    def getOrElse(key: Exp, default: (Sym, List[Sym])) =
...
class GraphBuilderOpt extends GraphBuilder {
  ...
  override def gatherEffectDepsWrite(
      s: String,
      as: Seq[Def],
      lw: Sym,
      lr: Seq[Sym]
    ...
    findDefinition(latest(lw)) match {
      case Some(Node(_, "array_set", as2, deps))
          if (s == "array_set" && as.init == as2.init) =>
  ...
  override def rewrite(s: String, as: List[Def]): Option[Exp] =
    (s, as) match {
      ...
      case (
            "array_get",
            List(Def("staticData", List(Const(as: Array[_]))), Const(i: Int))
          ) =>
        as(i) match {
      ...
      case (
            "array_set",
            List(as: Exp, i, rs @ Def("array_get", List(as1: Exp, i1)))
          ) if as == as1 && i == i1 && {
            // rs is part of the list of read since the last write
            curEffects
              .get(as)
              .filter({ case (_, lrs) => lrs contains rs.asInstanceOf[Sym] })
              .isDefined
          } =>
        Some(Const(()))
      ...
      case ("array_set", List(as: Exp, i, rs)) if ({
            curEffects
              .get(as)
              .flatMap({ case (lw, _) => findDefinition(lw) }) match {
              case Some(Node(_, "array_set", List(as2, idx2, value2), _))
                  if (as == as2 && i == idx2 && value2 == rs) =>
                true
              case _ => false
            }
          }) =>
      ...
      case (
            ...
            List(
              ...
              rs @ Def(
                "reffield_get",
                List(Def("array_slice", (as1: Exp) :: idx1 :: _), i1)
              )
            )
          ) =>
      ...
      case (
            "reffield_set",
            List(as: Exp, i, rs @ Def("reffield_get", List(as1: Exp, i1)))
          ) =>
      ...
      case ("array_get", List(as: Exp, i: Exp)) =>
        curEffects
          ...
          .flatMap({
            case (lw, _) =>
              findDefinition(lw) collect {
                case Node(_, "array_set", List(_, i2: Exp, value: Exp), _)
                    if i == i2 =>
                  value
              }
          })
      ...
      case ("!=", List(Const(a), Const(b)))             => Some(Const(a != b))
      case ("^", List(Const(a: Boolean), Const(b: Boolean))) =>
        Some(Const(a ^ b))
      ...
      case ("<=", List(Const(a: Double), Const(b: Double))) =>
        Some(Const(a <= b))
      // idea 1:
      ...
      case ("?", c :: (t: Block) :: (e: Block) :: _)
          if t.isPure && e.isPure && t.res == e.res =>
        Some(t.res)
      case ("?", (c: Sym) :: (t: Block) :: (e: Block) :: _)
          if t.isPure && e.isPure =>
        (t.res, e.res) match {
          case (Const(t: Double), Const(e: Double)) if t.isNaN && e.isNaN =>
            Some(Const(Double.NaN))
          // c && true or c || false => if (c) true else false
          // if (c) false else true
          case (Const(t: Boolean), Const(e: Boolean)) /* if t != e */ =>
            Some(if (t) c else reflect("!", c))
          case _ => None
        }
  ...
  override def reflect(s: String, as: Def*): Exp =
    (s, as.toList) match {
      ...
      case (op, List(a, b)) if a == b && sameArgReduce.contains(op) =>
        Const(sameArgReduce(op))
